...
// https://www.reddit.com/r/threejs/comments/1bztj34/help_please_react_three_rapier_car_physics/
...
function setupJointTest(){
  ...
  wheelBLBody = physicsWorld.createRigidBody(
    RAPIER.RigidBodyDesc.dynamic()
  ...
  wheelBLMotor = physicsWorld.createImpulseJoint(
  ...
  //physicsWorld.createCollider(carShape, carBody)
  physicsWorld.createCollider(wheelBLShape, wheelBLBody)
}
...
function updateKey(){
  ...
  if (keyMap['Space']) {
    //wheelBLMotor.configureMotorVelocity(1, 1000.0);
    //console.log(wheelBLMotor)
    //console.log(wheelBLMotor.rawAxis())
    //wheelBLBody.resetTorques(true);
    wheelBLBody.setAngvel({ x: 0.0, y: 0.0, z: 0.0 }, true);//stop wheel spin
  }
  let targetSteer = 0
  if (keyMap['KeyA']) {
    targetSteer += 0.6
  ...
  wheelBLMotor.configureMotorVelocity(targetVelocity, 2.0);
