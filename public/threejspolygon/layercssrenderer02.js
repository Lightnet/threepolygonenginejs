...
class CSSRenderer {
  ...
  setupCSSScene(){
    ...
    cssObject.position.z = 10;
    this.cssScene.add(cssObject);

    //const threejselement = div({style:`width:800px;height:640px;`});
    //van.add(threejselement, this.renderer.domElement);
    //threejselement.style.background = new THREE.Color(Math.random() * 0xffffff).getStyle();
    const webglRender = this.renderer.domElement;
    console.log(webglRender)
    webglRender.style.width = '1024px';
    webglRender.style.height = '720px';
    
    const cssThreejs = new CSS3DObject(webglRender);
